...
genai.configure(api_key='AIzaSyBvUFpkTnTAeRoAK7u2qGerWuhzpDrzsfs')
...
def predict_answer(query):
    ...
    if similarity_score < 0.6:
        try:
            # Gửi yêu cầu tới mô hình Gemini
            response = model.generate_content(
                f"Câu hỏi: {query}\nVui lòng cung cấp câu trả lời ngắn gọn bằng tiếng Việt.")
            matched_answer = response.text
            matched_link = ""
            matched_question = query  # Giữ nguyên câu hỏi gốc
            similarity_score = 0.0  # Đặt lại điểm tương đồng vì không dùng TF-IDF
        except Exception as e:
            matched_answer = f"Lỗi khi gọi Gemini API: {str(e)}"
            matched_link = ""
